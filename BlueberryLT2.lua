 ...
 ▓██  █▒░▒██  ▀█▄  ░██   █▌▓██ ░▄█ ▒▒██▒▒████ ░ ▒ ▓██░ ▒░░ ▓██▄        ▒██▒ ▄██▒██ ██░▓██░ ██▓▒
  ▒██ █░░░██▄▄▄▄██ ░▓█▄   ▌▒██▀▀█▄  ░██░░▓█▒  ░ ░ ▓██▓ ░   ▒   ██▒     ▒██░█▀  ░ ▐██▓░▒██▄█▓▒ ▒
   ...
   ░ ░░    ▒   ▒▒ ░ ░ ▒  ▒   ░▒ ░ ▒░ ▒ ░ ░          ░    ░ ░▒  ░ ░ ░▒  ▒░▒   ░▓██ ░▒░ ░▒ ░     
     ░░    ░   ▒    ░ ░  ░   ░░   ░  ▒ ░ ░ ░      ░      ░  ░  ░   ░    ░    ░▒ ▒ ░░  ░░       
      ░        ░  ░   ░       ░      ░                         ░    ░   ░     ░ ░              
     ░              ░                                               ░        ░░ Version 2019P PEFORMANCE OBFUSCATOR
...
Save Peformance : Yes 
Anti Ban : Yes
Better Sorter : Yes But Only For Pc
Anti Cheat : Yes
Mobile Support : Yes But Beta!
Version : Admin
...
local Window = Rayfield:CreateWindow({
    Name = "Blueberry Hub",
    LoadingTitle = "Blueberry Hub",
    LoadingSubtitle = "by Blueberryシ",
    ConfigurationSaving = {
       Enabled = false,
       FolderName = nil, -- Create a custom folder for your hub/game
       FileName = "Blueberry Hub"
    },
    Discord = {
       Enabled = true,
       Invite = "z7cwgv6p", -- The Discord invite code, do not include discord.gg/. E.g. discord.gg/ABCD would be ABCD.
    ...
    KeySettings = {
       Title = "Blueberry Hub",
       Subtitle = "Key System",
       Note = "Join the discord (discord.gg/z7cwgv6p)",
       FileName = "Blueberry Config",
       SaveKey = true,
       GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like Rayfield to get the key from
       Key = "AOSO-AISI-K129-KALOI-WAPS-KNTL"
    }
 })
...
local File = SavingSystem.Init("Blueberry Hub Data Init", "Blueberry Hub.lua") -- Find or create a file in the ExampleFolder folder called ExampleFile
...
local MenuGUI = game:GetService("Players").LocalPlayer.PlayerGui.MenuGUI
    ...
    UICornerOpen1.CornerRadius = UDim.new(0, 20)
    ...
    MenuGUI.Menu.Main.Heading.Text = "Xiyizi X V1"
    ...
    UICornerOpen3.CornerRadius = UDim.new(0, 20)
    UICornerOpen4.Parent = MenuGUI.Menu.Main.DropShadow
    UICornerOpen4.CornerRadius = UDim.new(0, 20)
...
for _, frame in ipairs(frames) do
    ...
    if frame:IsA("TextButton") then
        frame.BackgroundColor3 = Color3.fromRGB(17, 16, 17)
    end
...
local window = library:new("Blueberry Hub")
...
credits:Label("Owner: Blueberryシ#001")
credits:Label("GUI Dev : Blueberryシ#001")


credits:Button("Copy Discord Server",function()
    setclipboard("https://discord.gg/z7cwgv6p")
    Instance.new("Sound", game:GetService("SoundService")).SoundId = "rbxassetid://1788243907"; game:GetService("SoundService").Sound:Play()
OrionLib:MakeNotification({
    Name = "Blueberry Hub",
    Content = "Discord Invite Links Copied",
    Image = "rbxassetid://10044538000",
    Time = 7
})
...
local credits = creds:section("UI Customization & Settings",true)
    ...
    credits:Button("Destroy GUI",function()
        game:GetService("CoreGui")["XiyiziXNoSkid!"]:Destroy()
    end)

    credits:Keybind("Toggle UI", Enum.KeyCode.F, function(Value)
        ToggleUILib()
    end)

    credits:Toggle("Transparent UI", "", false, function(state)
        if state then
        game:GetService("CoreGui")["XiyiziXNoSkid!"].Main.Style = "DropShadow"
        else
            game:GetService("CoreGui")["XiyiziXNoSkid!"].Main.Style = "Custom"
        end
    end)
...
movement:Slider('Flight Speed', 'JumpPower Slider', 50, 50, 250,false, function(value)
    maxspeed = value
end)

local movement = Player:section('Humanoid',true)

movement:Toggle('Flight [Q]', 'Click Q To Fly', false, function(value)
    FlyingEnabled = value
end)
...
local WorldTab = window:Tab("World",'6026568213')
...
local slot = SlotTab:section('Land',true)
...
if SelectedTreeType == "LoneCave" and not Axe then
    ...
    if SelectedTreeType ~= "LoneCave" then
        ...
        for i = 1,SelectedTreeTypeQuantity do
            ...
            if FindTreeOfClass(SelectedTreeType) == false then NotifyInstance.new("Sound", game:GetService("SoundService")).SoundId = "rbxassetid://1788243907"; game:GetService("SoundService").Sound:Play()
                OrionLib:MakeNotification({
                    Name = "Blueberry Hub",
                    Content = "Please Select Item Type",
                    Image = "rbxassetid://10044538000",
                    Time = 7
                })  break end
            CuttingTree = true
...
OrionLib:MakeNotification({
    Name = "Blueberry Hub",
    Content = "Click Plank For 1x1 Plank Cut",
    Image = "rbxassetid://10044538000",
    Time = 7
})
...
wait(1)
local ItemTab = window:Tab("Item",'6031090999')
...
local SorterTab = window:Tab("Sorter",'6031090999')
...
sort:Slider("Y", "", 0, 0, 20, false, function(value)
    Y = value
end)

sort:Slider("Z", "", 0, 0, 25, false, function(value)
    Z = value
end)
...
OrionLib:MakeNotification({
    Name = "Blueberry Hub",
    Content = "Please select an item to start",
    Image = "rbxassetid://10044538000",
    Time = 10
})
  ...
  until SelectedItem~=nil
     ...
     local  Selection = Instance.new("SelectionBox", SortPreview)
      ...
      SortPreview.Transparency = 2
      SortPreview.CanCollide = false
      SortPreview.Anchored = true
      SortPreview.Name = "Blueberry Hub"
      ...
      selectpos = Mouse.Button1Down:Connect(function()
          ...
          game:GetService "Workspace":FindFirstChild "Blueberry Hub":Destroy()
          game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame=oldpos
          Instance.new("Sound", game:GetService("SoundService")).SoundId = "rbxassetid://1788243907"; game:GetService("SoundService").Sound:Play()
OrionLib:MakeNotification({
    Name = "Blueberry Hub",
...
sort:Button('Stop',function()
    ...
    OrionLib:MakeNotification({
        Name = "Blueberry Hub",
        Content = "Sorter Stopped",
        Image = "rbxassetid://10044538000",
        Time = 7
    })
end)

local TrollingTab = window:Tab("Trolling",'6034509993')

local troll = TrollingTab:section('Anti-Blacklist',true)

troll:Toggle("Bypass Blacklist", " ", false, function(state)
    if state then
        antiblacklist()
    else
        offantiblacklist()
...
local XiyiziX = window:Tab('Visuals','7044233235')

local Xyz = XiyiziX:section('Visuals',true)


Xyz:Textbox('Fake Money', 'TextBoxVal', "Type Money You Want", function(value)
    game.Players.LocalPlayer.leaderstats.Money.Value = value
end)

Xyz:Toggle("Gold Blueprint", " ", false, function(value)
    game.Players.LocalPlayer.SuperBlueprint.Value = value
end)

---logs---
local UpdateLogs = window:Tab('Dev Logs','6962520787')

---version
local ASM = UpdateLogs:section('Version 2015E',true)
local ASC = UpdateLogs:section('Version 2015D',true)
local ASV = UpdateLogs:section('Version 2015C',true)
local ASB = UpdateLogs:section('Version 2015B',true)
local ASN = UpdateLogs:section('Version 2015A',true)
...
ASM:Label("Reworked UI Libs,Credit To Underscore")
ASM:Label("Better Sorter And Make Sorter Support Krnl")
ASM:Label("Small Rework Auto Buy")
ASM:Label("Optimizing the Script When You Just Execute")
ASM:Label("Fixing Cant Teleport To Player")
ASM:Label("Fixing Sell All Planks Bug")
ASM:Label("Making Sorter Support Mobile (Fluxus Mobile)")
ASM:Label("Optimizing For Mobile Executor")

ASC:Label("[+] Adding Max Land Dupe")
ASC:Label("[+] Change Ui Libs To Silent Ui Libs")
ASC:Label("[+] Better Auto Buy Speed")
ASC:Label("[+] Adding Graphics Enchanter (Beta)")
ASC:Label("[*] Fixing Stuck In Console")
ASC:Label("[-] Deleting Item Tab (We Want Rework This Feature)")

ASV:Label("[*] Fixing Bug")

ASB:Label("[+] Updated Item Sorter")

ASN:Label("[+] New Feature Item Teleports")
    ASN:Label("[*] Better Auto Buy")
...
OrionLib:MakeNotification({
    Name = "Did You Know?",
    Content = "The Best Lumber Tycoon 2 Shop Is Blueberry Hub's",
    Image = "rbxassetid://10044538000",
    Time = 7
})
---if xiyizix srvr crash then

wait(60)
Instance.new("Sound", game:GetService("SoundService")).SoundId = "rbxassetid://1788243907"; game:GetService("SoundService").Sound:Play()
OrionLib:MakeNotification({
    Name = "Blueberry Hub",
    Content = "Dont Forget To Join Blueberry Hub Discord!",
    Image = "rbxassetid://10044538000",
    Time = 7
})
